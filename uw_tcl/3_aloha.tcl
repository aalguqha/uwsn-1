...
set opt(tz)		10
set opt(seed)		11
set opt(tr)		"data/3_aloha.tr"	;# trace file
set opt(datafile)	"data/3_aloha.data"	;# data file
set opt(nam)            "data/3_aloha.nam" 	;# nam file
set opt(adhocRouting)   Vectorbasedforward
set opt(width)          100
set opt(range)          100    			;#range of each node in meters

set opt(stop)		1500			;# simulation time
set opt(prestop)        1495     		;# time to prepare to stop
set opt(packet_size) 	500  			;#50 bytes

set opt(interval)       5.0
Mac/UnderwaterMac set bit_rate_ 300.0e4 	;#10kbps
...
Mac/UnderwaterMac/UWALOHA set AckSizeRatio 4.0			;#ack的大小
...
Phy/UnderwaterPhy set freq_  	50  	;# 25khz  
...
$a_(5) cmd set-target-x $opt(tx)
$a_(5) cmd set-target-y $opt(ty)
$a_(5) cmd set-target-z $opt(tz)
