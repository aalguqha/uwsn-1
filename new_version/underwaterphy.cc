...
/*,sense_handler(this)*/
{
	...
	*/
	...
	bind("fowarding_delay", &fowarding_delay_);
        ...
        bind("waterdepth_", &waterdepth_); //by zuba
        bind("soundspeeddiff_", &soundspeeddiff_);//by zuba
        bind("sedimenttype_", &sedimenttype_);//by zuba
        bind("shallowwatermodel_", &shallowwatermodel_);//by zuba
        bind("shipping_", &shipping_);//by zuba
        bind("wind_", &wind_);//by zuba
        bind("noisemodel_", &noisemodel_);//by zuba
...
//changed by yishan su for OFDM module test
		//if(modulation_) {
		//	hdr_cmn *hdr = HDR_CMN(p);
		//	hdr->error() = modulation_->BitError(Pr);
		//}

                UW_OFDM *obj=new UW_OFDM;
                UnderwaterPropagation *mo=new UnderwaterPropagation;
                printf("underwaterphy: Pr receive is %f\n",Pr);
                double SNR_value=mo->SNR(Pr,this);
                printf("underwaterphy: SNR_value receive is %f\n",SNR_value);
                if(1) { 
			hdr_cmn *hdr = HDR_CMN(p);
                           printf("go OFDM_MODULATION\n");
			hdr->error() = obj->BitError(SNR_value);
                        delete obj;
		}
...
//add by yishan su 2012.12.6 for modulation module

int UnderwaterPhy::modulation_decision(double sinr)
{
 double lower_threshold=0;
 double upper_threshold=0;
 double packet_error_rate=0;
 double x=0;
 if(sinr<lower_threshold)
    return 1; //not receive

 else
    if(sinr>upper_threshold)
       return 0; //receive
    else
     { packet_error_rate=(sinr-lower_threshold)*1/(upper_threshold-lower_threshold);//simplify by linea
       x=Random::uniform();
       if (x<packet_error_rate)
           return 0;
       else
           return 1;
     }
