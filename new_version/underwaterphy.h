...
public:
	...
	inline double forwarding_delay() { return fowarding_delay_; }
        ...
        inline double getWaterDepth() { return waterdepth_; }     //get water depth in meters by zuba
        inline double getSoundSpeedDiff() { return soundspeeddiff_; }     //get sound speed difference by zuba
        inline double getSedimentType() { return sedimenttype_; }        //get sediment type by zuba
        inline double getShallowWaterModel() { return shallowwatermodel_; } //set shallow water model use by zuba
        inline double getShippingNoise(){ return shipping_;} //get shipping noise by zuba
        inline double getWindNoise(){ return wind_;} //get wind noise by zuba
        inline double getNoiseModel(){ return noisemodel_;} //set noise model by zuba
	...
	int DefaultBitError(int pktsize);
        int modulation_decision(double sinr); //add by yishan su 2012.12.6
...
protected:
	...
	//transition delays
        ...
        double waterdepth_;         //water depth in meters by zuba
        double soundspeeddiff_;     //sound speed difference by zuba
        double sedimenttype_;        //specify sediment type by zuba
        double shallowwatermodel_;  //Use shallow water model by zuba
        double shipping_;        //specify shipping noise by zuba
        double wind_;            //specify wind noise by zuba
        double noisemodel_;      // by zuba
